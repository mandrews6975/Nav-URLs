...
// Deprecated (for purposes of updating naming convention)
...
exports.getRouteURL = (locations, options) => {
  let url = '';
  if (locations.length < 2) {
    console.log('Invalid locations array: ' + locations.toString());
    return;
...
exports.getOptimizedRouteURL = async (locations, key, options, callback) => {
  if (options.platform == 'apple') {
    console.log('Invalid platform specification: ' + options.platform);
    return;
  }
  try {
    fetch('http://www.mapquestapi.com/directions/v2/optimizedroute?key=' + key, {
        method: 'post',
        headers: {
      ...
      .then((res) => res.json())
      .then((json) => {
        if (json.route.locationSequence) {
          let optimizedLocOrder = json.route.locationSequence;
          let optimizedLocations = [];
          for (let i = 0; i < locations.length; i++) {
            optimizedLocations.push(locations[optimizedLocOrder[i]]);
          }
          if (callback) {
            callback(exports.getRouteURL(optimizedLocations, options));
          }
        } else {
          console.log('Response from MapQuest API was invalid');
          return;
        }
      });
  } catch (error) {
